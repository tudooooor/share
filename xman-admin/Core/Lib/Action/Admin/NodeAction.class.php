...
class NodeAction extends AdminAction {
	...
	public function index(){
		...
		foreach($Node as $k => $r) {
			...
			$r['submenu'] = $r['level']==3 ? '<font color="#cccccc">添加子菜单</font>' : "<a href='".U('Admin/Node/add',array('pid'=>$r[id]))."'>添加子菜单</a>";
			$r['edit']    = $r['level']==1 ? '<font color="#cccccc">修改</font>' : "<a href='".U('Admin/Node/edit',array('id'=>$r['id'],'pid'=>$r['pid']))."'>修改</a>";
			$r['del']     = $r['level']==1 ? '<font color="#cccccc">删除</font>' : "<a onClick='return confirmurl(\"".U('Admin/Node/del',array('id'=>$r[id]))."\",\"确定删除该菜单吗?\")' href='javascript:void(0)'>删除</a>";
			switch ($r['display']) {
				case 0:
					$r['display'] = '不显示';
					break;
