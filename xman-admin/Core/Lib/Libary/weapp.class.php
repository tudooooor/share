...
class Weapp
{
    ...
    const COM_URL_PREFIX = 'https://api.weixin.qq.com';
    ...
    const QRCODE_CREATE_URL='/wxa/getwxacode?';
    const QR_SCENE = 0;
    ...
    const TEMPLATE_ADD_TPL_URL = '/wxopen/template/add?';
    const TEMPLATE_LIST_TPL_URL = '/wxopen/template/list?';
    const TEMPLATE_SEND_URL = '/message/wxopen/template/send?';
    const TEMPLATE_GET_TPL_URL = '/wxopen/template/library/get?';
    ...
    const OAUTH_TOKEN_URL = '/sns/jscode2session?';
    ...
    const WXAAPP_CREATEWXAQRCODE_URL = "/wxaapp/createwxaqrcode?";
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function createQrcode($path,$width,$appid='',$appsecret='',$token='') {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::WXAAPP_CREATEWXAQRCODE_URL . "access_token=" . $this->access_token, json_encode(array('path' => $path,'width' => $width)));
        if($result){
            $json = json_decode($result,true);
...
//    }
     ...
     *
     */
    public function getQRCode($path,$width,$auto_color,$line_color) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array();
        ...
        $result = $this->http_post(self::COM_URL_PREFIX.self::QRCODE_CREATE_URL.'access_token='.$this->access_token,self::json_encode($data));
    ...
    public function getOauthAccessToken() {
        ...
        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::OAUTH_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&js_code='.$code.'&grant_type=authorization_code');
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function getTemplate($tpl_id) {
        $data = array ('id' =>$tpl_id);
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_GET_TPL_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function templateList($offset=0,$count=20) {
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_LIST_TPL_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     */
    public function addTemplateMessage($tpl_id,$keyword_id_list){
        $data = array ('id' =>$tpl_id,'keyword_id_list' => $keyword_id_list);
        if (!$this->access_token && !$this->checkAuth()) return false;
