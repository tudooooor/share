...
Page({
  ...
  getNowTime: function () {
    ...
    if (day < 10) { day = '0' + day; }; // 如果需要时分秒，就放开 // var h = now.getHours(); // var m = now.getMinutes(); // var s = now.getSeconds(); 
  ...
  {
    var that = this;
    var url = this.baseApiUrl + "?g=Api&m=Weuser&a=getStatDataYear&token=" + this.token;
    util.ajax({
      "url": url,
      "data": {
        "offset": 0,
